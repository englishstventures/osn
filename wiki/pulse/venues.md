...
- **P-W28** — replace `GET /venues` (and `listEvents`) with a shared bbox query contract `(minLat, maxLat, minLng, maxLng, limit)`; indexes already exist. Open as `P-W28` in `englishstventures/osn-tracker`.

...
| `@osn/auth-ui` / `@musubi/social` | `<TotpView>` in Settings → Security; TOTP factor in `<StepUpDialog>`; "email me a code" on `<RecoveryLoginForm>`. |
...
| 6 | `@osn/auth-ui` + `@musubi/social` surfaces for TOTP and email recovery | 3 |

...
| `@cire/invites` Pinterest embed | guest IP / UA / behaviour (unless the guest switches third-party content off) | ePrivacy + GDPR — consent-gated under the `embeds` category (opt-out), outbound link shown whenever the board is not; Pinterest subprocessor row + transfer basis TODO ([[subprocessors]]) |
...
| CC5 — Control Activities | This page, the open findings in `englishstventures/osn-tracker` | Quarterly access review record |

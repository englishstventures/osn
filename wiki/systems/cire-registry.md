...
**The ceiling is said in words, at the box that enforces it.** The quantity input carries a real `max`, and `giftRegistryQuantityHint` puts the reason beside it ("You can reserve up to 2.", "You can reserve 1.") whenever the ceiling is below the wedding-wide 99 — a `max` a guest cannot see only turns into a number silently clamped on submit. It is wired with `aria-describedby`, conditional on there being a hint, and sits **outside** the wrapping `<label>`: the label's whole text is the input's accessible name, so a hint inside it would be read out as part of the name. It is not a scarcity badge — it appears only once a guest has chosen to reserve and is picking how many, so it bounds an action rather than ranking the gift.

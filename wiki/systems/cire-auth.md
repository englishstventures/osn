...
A **`helper`** is someone handed a job on the day — carry the cake, mind the rings — not a co-organiser. They sit **below `viewer`**: the run sheet and nothing else, so no guest list, no budget, no registry, no vendors, no RSVPs. Nothing can assign the role yet. `HostRoleSchema` (`cire/api/src/schemas/host.ts`) is still `editor | viewer`, and `hostsService.add()` / `setRole()` take `AssignableHostRole`, so the compiler refuses a route that tries; the dropdown that widens it is its own issue and lands after this.

### The run-sheet scope

`wedding_hosts.run_sheet_scope` is `own | full`, **`own` by default** (migration `0059`; every seat that predates it back-filled to `own`). It is a helper's setting and no other role's — `runSheetScopeFor()` returns `full` for an owner, editor and viewer whatever the column says, because those three already read the whole dashboard and narrowing their run sheet would hide something they can reach another way.

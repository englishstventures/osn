...
`weddings.owner_osn_profile_id` stores the owning OSN profile id as an **opaque string** — no cross-DB FK (cire's D1 and OSN's DB are separate databases; the id is a foreign-system reference, not a relation). Four per-wedding gates enforce it, split by authorisation level:

> [!important] No gate enumerates the roles it turns away
> Each gate asks `decideCapability(role, capability)` and `policyFor()` answers (`cire/api/src/middleware/wedding-role.ts`). That switch is exhaustive over `WeddingRole` with no `default`, and `HostRole` is `Exclude<StoredHostRole, "host">` — derived from the Drizzle column rather than written out beside it. So **adding a value to `wedding_hosts.role` fails `bun run check` until every gate has been given a decision about it.**
>
> That is a guard, not a style preference. A gate that names the roles it excludes admits every role added after it: `weddingMember()` excluded nothing at all and `weddingEditor()` excluded `viewer` by name, so a new role would have inherited the whole read surface — guests, budget, registry, vendors, RSVPs — with no test failing. Add a role by adding it to the column and letting the compiler say where to decide.
...
- **`weddingEditor()`** — **owner OR `editor` co-host**, for the module WRITE surface (`cire/api/src/middleware/wedding-editor.ts`): the **change** routes (`/changes/head`, `/preview`, `/apply`, `/revert`, `/list` — the whole subtree is write-shaped: even preview persists a change row, and `head` exists only to seed an editor draft), the **invite-builder writes** (`PUT /invite/text`, `PUT /invite/theme`, image upload/delete/crop, event images), the per-event **location** write (`PUT /events/:eventId/location`) and its **geocode** helper (`POST /settings/geocode` — billed upstream call). It also fronts **`PUT /settings`**, which is not a whole-route decision: the middleware says who may reach the handler, and the handler then rejects a NON-OWNER patch touching anything but the RSVP-by deadline with **403 `owner_only_fields`** (naming the offending keys). Shape is decoded first, so a co-host's malformed date is still a 400; the refusal is whole, never a partial write, and never a silent filter. A `viewer` co-host gets **403 `read_only_role`** (a distinct error string so the portal can say "ask the owner for editor access"); a `helper` gets plain **403 `forbidden`**, since neither half of that advice is true for them; non-members get the member gate's 404/403 semantics unchanged.
- **`weddingMember()`** — **owner OR a co-host carrying the `member` capability (`editor`, `viewer` — NOT `helper`)**, for the read surface: the dashboard reads (`/guests`, `/events`, `/rsvps`), the CSV exports, `GET /settings`, the co-host read route (`GET /hosts`), the invite read (`GET /invite`), and `preview-code` (previewing is the read experience — see above). Resolves authorisation via `hostsService.authorize()` — one query for the owner (owner id and slug), and a second for anyone else (the host-row probe, incl. the seat's role) — so it both admits the caller and exposes `weddingIsOwner` + `weddingRole` for any route that wants to keep a higher-privilege affordance inside a member-gated tree. Same 404/403 semantics; **fails closed** if the host/ARC lookup is unavailable. Sets `c.var.weddingId` + `c.var.weddingIsOwner` + `c.var.weddingRole` + `c.var.weddingSlug`; the CSV exports (the download's filename), `GET /events` (event-image paths) and `preview-code` (the preview link) take the slug from `weddingSlug` rather than reading the wedding row a second time. A `helper` is refused here with the same **403 `forbidden`** a stranger gets — deliberately not `read_only_role`, which would both mis-advise the portal and confirm the seat exists.
- **`weddingRunSheet()`** — **owner OR a co-host carrying the `runSheet` capability**, which is every role including `helper` (`cire/api/src/middleware/wedding-run-sheet.ts`). It is the gate for the day-of run sheet, and it is **standalone: mount it INSTEAD OF `weddingMember()`, never after it.** Stacked behind the member gate a helper is refused there and never arrives, which would leave the run sheet the one thing a helper cannot read; it runs its own `hostsService.authorize()` for that reason. Beyond the usual context it derives `weddingHostId` (the caller's own `wedding_hosts.id`, absent for the owner) and `weddingRunSheetScope` — see the run-sheet scope below. Takes no `entitlementKey`. **No route mounts it yet**; the run-sheet view is a later sub-issue of the run-sheet epic.

**Roles capability matrix** (platform-plan §3.5). `wedding_hosts.role` is `editor | viewer | helper` (app-layer enum — no DB CHECK, so adding a value is not a data migration; migration `0031` rewrote the legacy `'host'` rows to `'editor'`, and readers normalise any stray `'host'` — still the column's DDL DEFAULT, unchangeable without a rebuild — to `editor`). Any OTHER unknown or corrupted value degrades to **the lowest-ranked role in the enum**, which `normaliseHostRole` reads off a rank map rather than naming: a literal `viewer` there would have outranked `helper` the moment `helper` landed, and a corrupted row would have kept the whole read surface.

| Action | Owner | Editor | Viewer | Helper | Gate |
|---|---|---|---|---|---|
...
A **`helper`** is someone handed a job on the day — carry the cake, mind the rings — not a co-organiser. They sit **below `viewer`**: the run sheet and nothing else, so no guest list, no budget, no registry, no vendors, no RSVPs. It is assigned like any other seat role, from the co-host dropdown in the organiser portal.

Which stored values a route may write is `ASSIGNABLE_HOST_ROLES` (`cire/api/src/services/hosts.ts`): one answer per value the column is declared to hold, so a value added to the column has to be told whether it can be assigned before the package compiles, and cannot become assignable by silence. `host` is answered `false` there permanently — it is the DDL default and the pre-roles value, not a role anyone holds. `hostsService.add()` / `setRole()` take the union derived from that map, which makes the compiler the bar on what a route can write, independently of the runtime bar `HostRoleSchema` (`cire/api/src/schemas/host.ts`) puts on the request body.

### The run-sheet scope

`wedding_hosts.run_sheet_scope` is `own | full`, **`own` by default** (migration `0060_helper_role_and_run_sheet_scope.sql`; every seat that predates it back-filled to `own`). It is a helper's setting and no other role's — `runSheetScopeFor()` returns `full` for an owner, editor and viewer whatever the column says, because those three already read the whole dashboard and narrowing their run sheet would hide something they can reach another way.

No route writes the column and `GET /hosts` does not return it, so every seat holds the default and a helper sees the tasks assigned to them.

**The filter is server-side and that is the point.** `runSheetVisibleTo()` (`cire/api/src/middleware/wedding-role.ts`) narrows the response body; the gate hands a route `weddingRunSheetScope` and `weddingHostId` so it cannot be written without them. A route that returned the whole run sheet and let the page show part of it would have given a helper the whole run sheet — it is one network tab away. An unassigned row is nobody's, so `own` excludes it.

The rationale: the spreadsheet is the primary way a wedding's guests + events are populated, and the invite is the thing the couple shapes together — locking either to the single owner defeated the point of co-hosting, and a hired *wedding planner* is exactly an `editor`. A `viewer` (a parent, a curious sibling) can watch RSVPs land without being able to change anything. The owner-only line is drawn at *managing the wedding itself*: rotating/cutting off guest credentials, wedding identity + money settings, changing who the co-organisers are, and deleting the wedding. **Adding a co-host** is the second place that line bends, and it bends on the additive-versus-subtractive axis rather than the read-versus-write one. An editor may grow the team; only the owner may shrink or demote it. Two properties make that safe. First, `editor` is the ceiling of what anyone can grant — every assignable role ranks at or below it, and the owner is never rowed into `wedding_hosts` — so an editor adding an editor is adding a **peer**, never a superior, and there is no path to owner. Second, because removal and demotion stay owner-only, an editor cannot evict the owner's other co-hosts, cannot demote a rival, and cannot entrench themselves; every seat an editor creates is reversible by the one person who cannot be removed. The worst case is an unwanted addition the owner then deletes. It exists because the alternative was worse in practice: the owner was the single person who could bring anyone on board, so handing out claim codes bottlenecked on them even when three people were running the wedding. The same reasoning — and the same additive-only shape — as the account-linking route. The **RSVP-by deadline** is the one field inside owner-only Settings that an `editor` may write — it runs the wedding rather than describing it (the co-host chasing replies is exactly who needs to move the date, and nothing about it is irreversible), which is why the settings row above splits in two. See [[cire-rsvp-deadline]].

Co-hosts live in the `wedding_hosts(wedding_id, osn_profile_id, role, …)` table (unique per pair) — it stores **only the profile id**, never the handle. They're added **by OSN handle** — `POST /hosts` (body `{handle, role?}`, role defaults to **`viewer`**, the least a seat can be given; **owner or `editor`**) resolves the handle to a profile id via an ARC-gated osn-api `GET /graph/internal/profile-by-handle` call (`graph:read` scope) before inserting the row (#148); `PUT /hosts/:osnProfileId/role` sets a seat to any assignable role (404 `host_not_found` for non-seats, incl. the owner — the owner is never rowed in). The portal's wedding list (`GET /weddings`) tags each row with the caller's role (`owner | editor | viewer | helper`) so the UI can hide write/management surfaces; the API gates remain the enforcement.

### What the portal offers each role

The organiser portal decides every role-driven affordance in one place, `cire/host/src/lib/wedding-roles.ts`, and the components ask it rather than comparing a role themselves. `surfacesFor()` is a switch over the whole vocabulary with no `default`, so a role added there fails `bun run check` until each decision has answered for it.

| Portal surface | Owner | Editor | Viewer | Helper | Mirrors |
|---|---|---|---|---|---|
| The wedding dashboard (`ModuleShell`), the invite-preview button, the palette's module rows | ✅ | ✅ | ✅ | ❌ | `weddingMember()` |
| The module write surfaces, and the add-a-co-host form | ✅ | ✅ | ❌ | ❌ | `weddingEditor()` |
| The role dropdown and the remove control on a co-host row | ✅ | ❌ | ❌ | ❌ | `weddingOwner()` |

> [!important] The portal's list is not derived from the column
> `@cire/host` depends on neither `@cire/api` nor `@cire/db`, so a role added to `wedding_hosts.role` compiles the portal unchanged and every seat holding it renders as the lowest-ranked role the portal knows. `cire/host/tests/lib/wedding-roles.contract.test.ts` reads the column out of `cire/db/src/schema.ts` and fails when the two lists disagree — that test is the link, not the type system.

A co-host is added as a **viewer** and the role is set afterwards from a dropdown on their row, so adding someone and deciding what they may do are separate acts. Moving a seat **up to `editor`** — the widest a seat can be given — asks for confirmation first; a demotion does not, being reversible by the same person in the same place. An unrecognised role degrades to the lowest rank rather than to a named role, so a role added below the current floor moves the floor with it.
...
Both cookies now sit on the same host (`api.cireweddings.com`), so a browser holding both sends both on every call — but they are read by name and stored in **different tables**: `cire_session` → `sessions` (family-scoped), `cire_org_session` → `organiser_sessions` (profile-scoped). `sessionAuth()` never looks at the organiser cookie and `osnAuth()` never looks at the guest one, so co-residency grants nothing: a leaked guest cookie can never reach organiser surface and an organiser cookie is meaningless on the RSVP endpoint. One read crosses over, and it authorises nothing: when a household is offered account linking, `POST /api/claim` and `GET /api/claim/session` check whether the request's `cire_org_session` is live and report the answer as one boolean (`accountLink.signedIn`, see [Guest account linking](#guest-account-linking-the-one-deliberate-dual-credential-route)). They return nothing about the session's profile, act on nothing, and never read it for a host-preview family. The interim `X-Organiser-Token` shared secret that predated this model is fully deleted.
...
| | |
|---|---|
| Endpoints | `POST /api/account/link` (dual-credential), `DELETE /api/account/link/:guestId` (guest-only) |
| Link state | Read through the claim payload (`accountLink` on `POST /api/claim` and `GET /api/claim/session`), not a route of its own |
| Middleware | `sessionAuth()` (Elysia plugin) on both methods; `osnAuth()` **method-gated to POST** by mounting it on a sibling Elysia instance — `createAccountLinkPostRoute` (POST, sessionAuth + osnAuth) and `createAccountLinkRoutes` (DELETE, sessionAuth only) share the prefix but not the OSN gate, the same sibling-instance pattern that keeps `/api/rsvp` ungated by the organiser `osnAuth` |
...
**401 here is correct (no 403 hazard).** Unlike the organiser 403-vs-401 rule above, a **401 from `osnAuth()` on the link POST is the right answer**: the guest's cire session for their OSN identity has expired, and the right response is to send them back through the sign-in redirect. There is no authorisation wall to mask — the guest is *re-authenticating to OSN*, not being denied a cire resource. `DELETE` never invokes `osnAuth()` (a guest whose OSN sign-in lapsed can still remove their household's links), so it can't trip it.

**ARC on Workers.** cire/api runs on workerd, so it mints the outbound ARC token via the DB-free, metric-free `@shared/crypto/jwk` `signArcToken` (the barrel `@shared/crypto` and `@shared/observability` don't bundle for workerd). Key distribution is a **stable** ES256 key (`CIRE_API_ARC_PRIVATE_KEY` wrangler secret) pre-registered in osn-api's `service_accounts` under serviceId `cire-api` — not the ephemeral-key self-registration + rotation that long-lived bun services use, because a Worker has no startup hook. When the ARC key is absent the POST answers **503** and the claim payload reports linking as off, so the box is never offered (linking is opt-in; the rest of cire is unaffected). The resolver is injectable (`createApp({ resolveOsnAccountId })`) so tests stub it.
...
1. The claim and restore responses carry the household's state as `accountLink`: `{ enabled: false }`, or `{ enabled: true, signedIn, linkedGuestIds }` (never an account or profile id). The box draws from that alone and makes no request to appear, so it arrives with the welcome panel rather than a request later, above events already on screen. `enabled` needs `cire.account-linking` on for the household ([[feature-flags]]), an ARC resolver, and a guest family (never host preview). The payload waits at most 250 ms for the flag; past that, or on any failed read, it reports `enabled: false` and the invite opens without the box. The guest site reads the field through `readAccountLink`, so a missing or malformed value hides the box and never the invite.
2. Signed out, it offers "Sign in with musubi" — `startSignIn` from `@shared/rp-auth`, the same top-level redirect the organiser portal uses. The guest cookie survives the round trip, so the guest comes back to the claimed invite, and the restore reports them signed in.
3. Signed in, the guest picks **which household member they are** and the panel POSTs `{ guestId }` through the auth client's `authFetch`. **409 is treated as success**, not an error: the seat is linked either way, and surfacing "already linked" as a failure would be a lie. 403 → "That isn't one of your household's guests." A thrown `AuthExpiredError` flips the panel back to the sign-in button; a network failure keeps the picker and asks for a retry.

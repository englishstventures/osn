...
**Both** cookies carry auth state — `cire_session` since the beginning, `cire_org_session` since the OIDC swap — so cire needs CSRF defence beyond `SameSite=Lax`, and the organiser surface now needs it as much as the guest one (before the swap it was Bearer-only and therefore CSRF-immune by construction). A root-level Elysia `onBeforeHandle` (`cire/api/src/lib/origin-guard.ts`, mounted in `createApp` before the route factories) validates the `Origin` header on **every state-changing method** (POST/PUT/PATCH/DELETE) against the same allowlist CORS echoes (derived from `WEB_ORIGIN`). Missing or mismatched Origin → **403** with a bounded `cire.origin_guard.rejections{reason}` metric (`missing | mismatch`). Four routes sit outside the guard, and no browser session authenticates any of them: the CSP violation collector and the internal session-revoke back-channel are mounted before it, and the two Stripe webhook paths (`/api/stripe/webhook`, `/api/stripe/platform-webhook`) are exempt by exact path, because Stripe sends no `Origin` and each route checks its own signature. Every other state-changing request is checked. An empty allowlist (local dev) disables the guard.

**The CORS preflight allows `GET, POST, PUT, PATCH, DELETE, OPTIONS`** — the `methods` list in the `cors()` call in [`cire/api/src/app.ts`](../../cire/api/src/app.ts): every method a route answers, and never `*`. The portals call the API cross-origin with credentials, so each non-GET call is preflighted and the browser refuses any method the list leaves out. CORS matches the request's `Origin` by exact membership in the same allowlist the guard uses, through a function passed as `origin`: a plain string list would go through `@elysiajs/cors`'s own matcher, which strips everything up to `://` and consults a plain object's inherited keys. [`cire/api/tests/app.test.ts`](../../cire/api/tests/app.test.ts) sends a PATCH preflight from every origin in both tiers' committed `WEB_ORIGIN`, checks that an unlisted or near-miss origin gets no `Access-Control-Allow-Origin`, and fails unless the list names exactly the methods the mounted routes answer. [`cire/api/tests/lib/origin-guard.test.ts`](../../cire/api/tests/lib/origin-guard.test.ts) checks the guard refuses a PATCH, PUT or DELETE to an organiser route whose `Origin` is missing or unlisted.

...
- **The native binding counts per Cloudflare location**, not globally ([[rate-limiting]]). The deployed Worker uses `REGISTRY_PREVIEW_RATE_LIMITER` (`simple = { limit = 10, period = 60 }` in `cire/api/wrangler.toml`); without it the in-memory default counts per isolate, which a caller can widen by landing on more isolates.
...
- **There is no daily total.** Nothing caps what one profile spends over a day. The route sits behind an editor seat on a wedding that holds the paid `registry` entitlement, so a caller who abuses it is known, not anonymous. Add a per-profile daily byte or request budget on top of the per-minute limiter when either changes: link preview reaches a surface cheaper than a paid registry, or the subrequest count or `cire.registry.link_preview` shows preview traffic out of line with hand use. The native binding cannot hold it — its `period` is 10 or 60 seconds — so it needs a store that counts across isolates.

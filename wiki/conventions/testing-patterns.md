...
- **When tests need a wider type surface than the shipped source, give them `tests/tsconfig.json`.** It extends `../tsconfig.json`, widens what the tests need (`bun-types`, a later `lib`), and the package's `check` script runs it after the main config. The package's own `tsconfig.json` then covers only shipped source, so the compiler rejects what the runtime lacks. Name it `tests/tsconfig.json`, not `tsconfig.test.json` beside the main one: the editor looks for a file named `tsconfig.json` in the nearest ancestor directory, so a sibling config leaves test files on the repo-root `tsconfig.json`, which has no JSX or DOM settings. `cire/api`, `cire/invites` and `shared/db-utils` use it — [[cire-development#Type-check configs]].

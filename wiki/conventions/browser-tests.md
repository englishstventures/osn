...
| An `sr-only` control resolving against a distant positioned ancestor, so focusing it scrolls a whole dialog sideways (see [[frontend-patterns]] § Rendering and animation gotchas) | Requires containing blocks, scroll offsets and the browser's own focus-scroll                                                                                               |
| `overflow: hidden` still being a scroll container, and `overflow-x: clip` computing to `hidden` beside an `auto` axis                                         | Requires computed values and a real scroll offset                                                                                                                           |
...
| `@cire/invites`  | `tests/components/DietaryPresets.browser.test.tsx`      | The dietary picker's preset track overflows **inside** the sheet rather than widening it (a `<fieldset>` sizes to its content unless every box down to the scrollport may be narrower than what it holds), and the picker stays inline at desktop width — a `showModal()` dialog paints above every stacking context, so a portalled popover opened from inside it would be unreachable at any z-index; and ticking a pill after scrolling the track sideways leaves the sheet where it was, because each hidden `sr-only` checkbox resolves inside its own pill rather than against the `<dialog>` (unfixed: `dialog.scrollLeft` 0 → 1213 at 1024px, 0 → 1182 at 414px). Plus a hit test that the close chip still paints over sixteen now-positioned pills |
| `@cire/invites`  | `tests/components/DietaryOther.browser.test.tsx`        | The "Anything else" field opens on a real `grid-template-rows` transition when "Other" is ticked, is simply open with no transition for a reply that already carries prose, and leaves the document at once when "Other" is unticked — so nothing focusable survives inside a collapsing box                                                                                                                                                                                                                                                                                                         |
| `@shared/ui`     | `tests/modal.browser.test.tsx`                          | Among the platform behaviours `<dialog>` is chosen for: that a `frame` panel computes `clip` on **both** axes and so is not a scroll container — a `scrollIntoView` on a descendant outside it leaves `dialog.scrollLeft` at 0, which `overflow: hidden` does not                                                                                                                                                                                                                                                                                                                                    |

...
| `@musubi/social` | `tests/styles/token-contract.browser.test.tsx` | A contract utility emits CSS **at all** (an unresolvable one emits nothing, silently); `bg-osn-accent` paints exactly what `--primary` holds; the destructive button's ink comes from `--destructive-foreground` rather than the `text-white` it used to hard-code; the mapping follows `.dark` because it is aliases and not literals; `base:` still compiles to `:where(…)`, so a call-site `class` still wins |
| `@pulse/web` | `tests/styles/token-contract.browser.test.tsx` | The same chain on a different ramp, plus the mapping decision that only a colour can check: `--osn-accent` paints `--primary` and **not** the coral `--pulse-accent`, so nobody can "fix" the mapping to the brand colour and repaint every shared button |

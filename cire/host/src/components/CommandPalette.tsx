        ...
        <div class="fixed inset-0 z-50 flex items-start justify-center p-4 pt-[12vh]">
          <Dialog.Content class="sheet-panel border-border bg-surface flex w-full max-w-lg flex-col overflow-hidden rounded-md border shadow-(--elev-2) outline-none">
            ...
            <div class="border-border flex items-center gap-3 border-b px-4">
              <span aria-hidden="true" class="text-gold text-osn-base shrink-0">
                ⌘
              </span>
              <input
                type="text"
                ...
                onKeyDown={onKeyDown}
                class="font-body text-text placeholder:text-text-faint text-osn-base min-w-0 flex-1 bg-transparent py-3.5 outline-none"
              />
              <kbd class="font-body text-text-faint border-border text-osn-xs tracking-osn-wider hidden shrink-0 rounded-sm border px-1.5 py-0.5 uppercase sm:block">
                Esc
              </kbd>
            </div>
            ...
            >
                  ...
                  <>
                    <Show when={heading(index())}>
                      {(group) => (
                        <li
                          role="presentation"
                          class="font-body text-text-faint text-osn-xs tracking-osn-widest px-3 pt-2.5 pb-1.5 uppercase"
                        >
                          {group()}
                        </li>
                      )}
                    ...
                    >
                      ...
                      <span class="text-osn-sm min-w-0 flex-1 truncate">{command.label}</span>
                      <Show when={command.hint}>
                        {(hint) => (
                          <span class="text-text-faint text-osn-xs hidden max-w-[45%] shrink-0 truncate sm:block">
                            {hint()}
                          </span>
                        )}
                      </Show>
              ...
              <Show when={results().length === 0}>
                <li
                  role="presentation"
                  class="font-body text-text-muted text-osn-sm px-3 py-6 text-center"
                >

...
import { allowPlusOne, eventIdsOf, guestNamed } from "../test-helpers/plus-one";
...
function del(guestId: string, cookie: string | null) {
...
describe("PUT /api/plus-one/:guestId", () => {
  ...
  it("403s the organiser's host preview", async () => {
    ...
    allowPlusOne(db, hostGuest.id);
    const res = await put(hostGuest.id, cookie, { firstName: "Sam" });
    expect(res.status).toBe(403);
    expect(plusOnesOf(hostGuest.id)).toHaveLength(0);
  });
});

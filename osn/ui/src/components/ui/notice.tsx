...
const BASE =
  "base:font-osn-body base:flex base:items-start base:gap-2.5 base:rounded-osn-sm " +
  "base:border base:p-4 base:text-osn-base base:leading-osn-relaxed";

const TONE = {
  danger: "base:border-osn-danger/20 base:bg-osn-danger/5 base:text-osn-danger",
  warn: "base:border-osn-warn/20 base:bg-osn-warn/5 base:text-osn-warn",
  success: "base:border-osn-success/20 base:bg-osn-success/5 base:text-osn-success",
  info: "base:border-osn-hairline base:bg-osn-surface/30 base:text-osn-ink-secondary",
      ...
      <Show when={mark()}>
        {(m) => (
          <>
            <span
              aria-hidden="true"
              class="base:w-3 base:shrink-0 base:select-none base:text-center base:leading-osn-relaxed"
            >
              {m().glyph}
            </span>

...
describe("the control box", () => {
  ...
  it("paints a focus ring that is the focus token and not the accent", () => {
    ...
    // `focus-visible:ring-osn-focus` emitting nothing would be invisible to
  ...
  it("turns the control's own border when it is marked invalid", () => {
    ...
    expect(badBorder).toBe(paintToken("--osn-danger"));
  });
  ...
  it("lets a caller's class win, because every default is zero-specificity", () => {
    const { getByRole } = render(() => <Input class="bg-osn-accent" />);
    expect(getComputedStyle(getByRole("textbox")).backgroundColor).toBe(paintToken("--osn-accent"));
  });
...
describe("Field", () => {
  ...
  it("paints the error message in the danger tone and speaks it", () => {
    ...
    expect(getComputedStyle(alert.querySelector("p")!).color).toBe(paintToken("--osn-danger"));
  });

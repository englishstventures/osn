  ...
  return (
    <div class="base:flex base:flex-col base:gap-1">
      <div class={clsx("base:flex base:items-center base:gap-2", local.class)}>
        <span class="base:text-ui-ink-secondary" aria-hidden="true">
          @
        </span>
        <Input
          type="text"
      ...
      <Show when={status() === "checking"}>
        <span class="base:text-ui-ink-secondary base:text-ui-sm">Checking…</span>
      </Show>
      <Show when={status() === "available"}>
        <span class="base:text-ui-success base:text-ui-sm">@{local.value} is available</span>
      </Show>
      <Show when={status() === "taken"}>
        <span class="base:text-ui-danger base:text-ui-sm">@{local.value} is taken</span>
      </Show>
      <Show when={status() === "invalid" && local.invalidMessage}>
        <span class="base:text-ui-danger base:text-ui-sm">{local.invalidMessage}</span>
      </Show>
      <Show when={status() === "error"}>
        <span class="base:text-ui-danger base:text-ui-sm">

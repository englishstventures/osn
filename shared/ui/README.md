...
- `@osn/auth-ui/Register` — email + handle + OTP + optional passkey
  enrolment, three-step flow (`details` → `verify` → `passkey` → `done`).
- `@osn/auth-ui/SignIn` — three-tab sign-in (passkey / OTP / magic link)
  driving the new `/login/*` endpoints. Calls `adoptSession` on success.
- `@osn/auth-ui/MagicLinkHandler` — invisible root-level helper that
...
picks up the classes from `@shared/ui` source files.

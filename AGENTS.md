...
Which name a new package takes is in `wiki/architecture/osn-and-musubi.md`; the full tree is in `wiki/architecture/monorepo-structure.md`.
...
**Deployed** on Cloudflare: identity on `musubi.social` (the `osn-api` Worker on `id.musubi.social`), cire on `cireweddings.com`. A merge to `main` deploys the dev tier (`*.dev.…`) at once; production waits for a human to approve the `production` GitHub Environment. See `wiki/runbooks/dev-environment.md`.
...
- Each issue carries one `product:` label, an org type (`Feature`, `Bug`, `Task`) and a `complexity:` rating set **before** work starts (the `rate-complexity` skill). The full label scheme is in `wiki/runbooks/github-issues-setup.md`; how to file a finding is in `wiki/conventions/review-findings.md`.
...
- **Effect.** Build the layer graph once, as a `ManagedRuntime` at boot; never `Effect.provide` a database or observability layer inside a per-request `runPromise`. The v3 forms that no longer compile are in `wiki/architecture/effect-v4-api.md`. TypeBox validates at the HTTP boundary and Effect Schema in services; never mix them. Frontends do not use Effect.
- **Observability.** No `console.*`, no raw OpenTelemetry constructors, no unbounded metric attributes. See `wiki/observability/overview.md`.
...
Database commands differ between cire and the rest; see `wiki/conventions/commands.md` and `wiki/apps/cire-development.md`.
...
`wiki/` is one Obsidian vault for the whole monorepo. Start at `wiki/index.md`, or search it:
...
| To... | Read |
|---|---|
| Write a backend route or service | `wiki/architecture/backend-patterns.md`, `wiki/architecture/schema-layers.md` |
| Write frontend code, or hit a rendering bug tests cannot see | `wiki/architecture/frontend-patterns.md` |
| Use or add a UI component, or colour one | `wiki/architecture/component-library.md`, `wiki/architecture/design-tokens.md` |
| Work on one app | `wiki/apps/<name>.md`; cire's build rules are in `wiki/apps/cire-development.md` |
| Understand accounts, sessions and sign-in | `wiki/systems/identity-model.md`, `wiki/systems/sessions.md` |
| Check a D1 or free-tier limit | `wiki/systems/d1-limits.md`, `wiki/runbooks/free-tier-limits.md` |
| Deploy, or debug a deployed Worker | `wiki/runbooks/production-deploy.md`, the `debug-workers` skill |

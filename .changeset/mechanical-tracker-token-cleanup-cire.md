...
Clean up the `house/no-tracker-ref-in-comment` mechanical majority (englishstventures/osn#924).
...
A handful of leftover `osn-tracker#N` citations that predated both this batch and the separate tracker-number-refs cleanup (englishstventures/osn#930) are also gone from `@cire/host` and `@cire/vendor`, using the same treatment established there.

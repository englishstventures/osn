...
The mapping itself sends `--osn-accent` to the neutral `--primary`, **not** to
...
it is used from. For the same reason `--osn-accent-soft` takes `--muted`
...
`--osn-hairline-strong` is mapped but waived: `--input` measures 1.10–1.31:1

...
| Component Library | Zaidan-style (shadcn for SolidJS) on Kobalte, in two layers: `@shared/ui` for what is general, `@cire/ui` for the house style that is genuinely cire's. Component defaults use `base:`-prefixed classes written directly in source — `:where(&)`, zero specificity, so a caller's **plain** utility wins and a `base:` one ties. Two class utils cover the rest: `clsx()` conditional joins, `cn()` only for arbitrary conflicts. | `[[wiki/architecture/component-library]]` |

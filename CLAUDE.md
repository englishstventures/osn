...
| Close out a finished branch when the session is nearly out of budget — the card alone, no analysis | `.claude/skills/retro-v2/SKILL.md` (`/retro-v2`) |
...
| Agent skills and their evals | A procedure an agent follows lives in `.claude/skills/<name>/SKILL.md`, and nowhere else. **One skill here is somebody else's; the rest are ours.** `dgreenheck/webgpu-claude-skill` carries the WebGPU and TSL guidance `@tools/lab` three.js work needs, and it is installed as `.agents/skills/webgpu-threejs-tsl/` with a `.claude/skills/webgpu-threejs-tsl` symlink to it and a content-hashing root `skills-lock.json`. Install another the same way — `npx skills add <owner>/<repo> --agent claude-code universal`, never by copying the text in, so `npx skills update` can move the pin. Name both agents: the list is space-separated, and with only one target directory the CLI copies the files instead of symlinking them. **The same commit must carry the `.github/CODEOWNERS` rules and the changeset-allowlist entries for `.agents/**` and `skills-lock.json`**, because that tree is someone else's instructions running with full agent permissions. Nothing of ours writes into it: `oxlintrc.json` ignores both `.agents` and the symlink path (it walks through the link and reports the same file twice), both lefthook pre-commit commands exclude `.agents/**`, and the `skill-eval.yml` quality loop skips a symlinked skill — a finding fixed in someone else's file either stops the next `npx skills update` or is thrown away by it, and nothing here recomputes the lock's hash to tell you which. The two `Effect-TS/skills` ones went with the v4 migration, since a migration skill has no second use and what it taught is in `[[wiki/architecture/effect-v4-api]]`. Claude Code invokes a skill as `/<name>`, so a wrapper in `.claude/commands/` buys nothing and only splits the procedure across two files that drift; the older commands still in that directory are the ones not yet converted. Skills are also what makes a procedure measurable: `.claude/` is a Tessl plugin (`.tessl-plugin/plugin.json`), and each scenario under `.claude/evals/<scenario>/` runs an agent with and without the skill and scores the gap. A scenario pins a real commit of this repo and builds its branch in `setup.sh`, and that commit ships whatever `.claude/` held at the time, so `setup.sh` deletes it — `exclude` in `scenario.json` does not. Every merged fix PR is a free labelled scenario — pin its parent SHA, one checklist item per finding it fixed. `.claude/settings.json` is committed, so its hooks travel to the remote environment — that is what makes a standing rule hold in a session that never reads a local config. Personal hooks and permissions belong in `.claude/settings.local.json`, which is gitignored; a personal untracked `settings.json` will block the pull that first brings the tracked one down. See `.claude/evals/README.md` |
